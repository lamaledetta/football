...
mins=`$exedir/psql ev --tuples-only -A -c "select 4.5*count(distinct(matchId)) from franceligue1.event where season='$season'"`
mins=`echo ${mins%%.*}`
###cp -f $HOME/git_repo/ws-rev/bin/credentials.json bin

$RUN_ON_EV -f $HOME/git_repo/ev-rev/sql/progressive.fra.sql -v mins=$mins -v season=$season
