...
--    and     expg is not null
    group by name, matchId)
        select name, stats, round(90.0*sum(expected)/sum(mins), 3) as per_match
        from asd
...
--    and     expg is not null
    group by name, matchId)
        select name, stats, round(90.0*sum(expected * 2/(1 + exp(-0.1*(poss-50))))/sum(mins),1) as per_match
        from asd
        where expected is not null
        group by name,stats
        having sum(mins) >= $mins
...
tail -n +2 /tmp/prog_per_player.csv >> $HOME/Dropbox/My\ xG\ deliveries/pro-tableau/csv-factory/uu_players_ita.csv

#/usr/bin/python $HOME/git_repo/ev-rev/python/uu_tableau_players.loader.py
