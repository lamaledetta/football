...
$RUN_ON_EV <<SQL
    ...
    matchId,
    coalesce(round(sum(expa) ,3),0) as expected,
...
;
    ...
    matchId,
    coalesce(round(sum(expg) filter (where events @> '{shotSetPiece}' or events @> '{shotOpenPlay}' or events @> '{shotCounter}'),3),0) as expected,
    min(minsPlayed) as mins
    from englandpremierleague.event
    where   season='$season'
--    and     expg is not null
