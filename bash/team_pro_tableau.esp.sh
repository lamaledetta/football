...
mins=`$exedir/psql ev --tuples-only -A -c "select 4.5*count(distinct(matchId)) from spainlaliga.event where season='$season'"`
mins=`echo ${mins%%.*}`
...
copy (
    ...
    teamName as name,
    'PPDA' as stats,
    round((select sum(1) from spainlaliga.event e where (e.events @> '{passAccurate}' or e.events @> '{passInaccurate}') and e.season=spainlaliga.event.season and e.opponentName=spainlaliga.event.teamName and e.x < 60)/sum(1) filter (where (events @> '{tackleLost}' or events @> '{tackleWon}' or events @> '{interceptionAll}' or events @> '{foulCommitted}') and x > 40)::numeric,1) as per_match
    from spainlaliga.event
    where   season='$season'
...
$RUN_ON_EV -f sql/progressive.esp.sql -v mins=$mins -v season=$season
cp /tmp/uu_teams_tableau.csv $HOME/Dropbox/My\ xG\ deliveries/pro-tableau/csv-factory/uu_teams_esp.csv
tail -n +2 /tmp/prog_per_team.csv >> $HOME/Dropbox/My\ xG\ deliveries/pro-tableau/csv-factory/uu_teams_esp.csv
