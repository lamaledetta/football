...
mins=`$exedir/psql ev --tuples-only -A -c "select 4.5*count(distinct(matchId)) from englandpremierleague.event where season='$season'"`
mins=`echo ${mins%%.*}`
cp -f $HOME/git_repo/ws-rev/bin/credentials.json json

$RUN_ON_EV -f $HOME/git_repo/ev-rev/sql/progressive.eng.sql -v mins=$mins -v season=$season
...
copy (
    ...
    teamName as name,
    'PPDA' as stats,
    coalesce(round((select sum(1) from englandpremierleague.event e where (e.events @> '{passAccurate}' or e.events @> '{passInaccurate}') and e.season=englandpremierleague.event.season and e.opponentName=englandpremierleague.event.teamName and e.x < 60)/sum(1) filter (where (events @> '{tackleLost}' or events @> '{tackleWon}' or events @> '{interceptionAll}' or events @> '{foulCommitted}') and x > 40)::numeric,1),0.0) as per_match
    from englandpremierleague.event
    where   season='$season'
...
cp /tmp/uu_teams_tableau.csv $HOME/Dropbox/My\ xG\ deliveries/pro-tableau/csv-factory/uu_teams_eng.csv
