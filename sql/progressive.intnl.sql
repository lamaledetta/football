begin;
    ...
    insert into temp_progress (
        ...
        select 
            ...
            0,
            minsplayed
            from InternationalUEFANationsLeagueA.event where season=:'season'
            and position!='GK'
            and x>40 
    ...
    insert into progression (
        name, stats, per_match, top
    ) ( 
        (select team_name as name, 'Progressive passes' as stats, round(sum(max_pass_x - min_pass_x)/count(distinct(match_id)),1) as per_match, 'team' as top from progress group by team_name order by team_name)
        union all
        (with asd as (select team_name as name, 'Progressive passes' as stats, sum(max_pass_x - min_pass_x)/count(distinct(match_id)) as per_match from progress group by team_name order by team_name)
            select 'Media del campionato' as name, 'Progressive passes' as stats, round(avg(per_match),1) as per_match, 'team' as top from asd)
        union all
        (select team_name as name, 'Progressive runs' as stats, round(sum(max_run_x - min_run_x)/count(distinct(match_id)),1) as per_match, 'team' as top from progress group by team_name order by team_name)
        union all
        (with asd as (select team_name as name, 'Progressive runs' as stats, sum(max_run_x - min_run_x)/count(distinct(match_id)) as per_match from progress group by team_name order by team_name)
            select 'Media del campionato' as name, 'Progressive runs' as stats, round(avg(per_match),1) as per_match, 'team' as top from asd)
    );        
...
--    update ranks set total=90*pass_x/mins_played where run_x=0;
    ...
    insert into progression (
        ...
        (select player_name as name, 'Progressive passes' as stats, sum(passes) as per_match, 'player' as top from ranks group by player_name order by per_match desc limit 30)
        union all
        (select player_name as name, 'Progressive runs' as stats, sum(runs) as per_match, 'player' as top from ranks group by player_name order by per_match desc limit 30)
        union all
        (select player_name as name, 'Progressive passes & runs' as stats, sum(passes+runs) as per_match, 'player' as top from ranks group by player_name order by per_match desc limit 30)
    );
