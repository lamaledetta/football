...
from _parameter_levels
where parameter='year'
);

drop table if exists _factors;

create table _factors (
       parameter		text,
       level			text,
       type			text,
       method			text,
...
from _parameter_levels npl
left outer join _basic_factors nbf
