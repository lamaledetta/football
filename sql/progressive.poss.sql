begin;
    ...
    insert into temp_progress (
        ...
        min_run_x, max_run_x,
        min_pass_x, max_pass_x, mins, possession
    ) (
        ...
        select 
            ...
            minsplayed,
            matchPossession
            from italyseriea.event where season=:'season'
            and position!='GK'
            and x>40 
    ...
    select
        ...
        possession
        from (
            select match_id, id, player_id, player_name, team_id, team_name, min_run_x, max_run_x, mins, possession,
            row_number() over(order  by match_id,id)   -
            row_number() over(partition by player_id order by match_id,id) as grp
        from t
      ) tt
      group by match_id, player_id, player_name, team_id, team_name, mins, possession, grp
      order by min(id)
    );
    ...
    insert into ranks (
        ...
        with tt as (
            select match_id, player_id, player_name, team_id, team_name, sum(max_run_x - min_run_x) as deltax, min(mins) as mins, min(possession) as poss from progress where max_pass_x=min_pass_x group by match_id,player_name,player_id,team_id,team_name
        )
        select player_id,player_name,team_id,team_name, round(90.0*sum(deltax * 2/(1 + exp(0.1*(poss-50))))/sum(mins),1), 0 from tt group by player_id,player_name,team_id,team_name having sum(mins)>:'mins'
    );
    ...
    insert into ranks (
        ...
        with tt as (
            select match_id, player_id, player_name, team_id, team_name, sum(max_pass_x - min_pass_x) as deltax, min(mins) as mins, min(possession) as poss from progress where max_run_x=min_run_x group by match_id,player_name,player_id,team_id,team_name
        )
        select player_id,player_name,team_id,team_name, 0, round(90.0*sum(deltax * 2/(1 + exp(0.1*(poss-50))))/sum(mins),1) from tt group by player_id,player_name,team_id,team_name having sum(mins)>:'mins'
    );
...
--    update ranks set total=90*pass_x/mins_played where run_x=0;
    ...
    copy(
        (select player_name as name, 'Progressive passes' as stats, sum(passes) as per_match from ranks group by player_name order by per_match desc limit 200)
        union all
        (select player_name as name, 'Progressive runs' as stats, sum(runs) as per_match from ranks group by player_name order by per_match desc limit 200)
        union all
        (select player_name as name, 'Progressive passes & runs' as stats, sum(passes+runs) as per_match from ranks group by player_name order by per_match desc limit 200)
