...
# DATA ORGANIZATION
#NEW
ppp <- home_data[grepl("passAccurate", home_data$events) & home_data$endx > 67,]
qqq <- away_data[grepl("passAccurate", away_data$events) & away_data$endx > 67,]

cnt <- ppp %>% count(playername) 
cnt <- cnt[order(-cnt$n),]

ppp$playername[ppp$playername != cnt$playername[1]] <- ""
#unlist(qqq$playername)

subtitle <- paste("Successful last-third passes: ", nrow(ppp), "-", nrow(qqq), " ", " ", " ","Most involved: ", cnt$playername[1], "(", cnt$n[1],")")

gg <- ggplot(ppp, aes(x=-0.52+endx/100,y=-0.5+endy/100,xend=-0.52+x/100,yend=-0.5+y/100))+ theme_bw(base_family = font_f) + annotation_raster(pitch, -Inf, Inf, -Inf, Inf) + geom_segment(colour="lightgrey", size=1) + geom_text(size=2, family=font_f, aes(label=playername, x=-0.52+x/100, y=-0.5+y/100)) + geom_point(pch=21, colour="black", size=1, alpha=0.75, aes(fill=c("homecol"))) + ylim(c(-0.47,0.47)) + xlim(c(-0.47,0.47)) + guides(fill=FALSE)
#gg <- ggplot(ppp, aes(x=-0.52+endx/100,y=-0.5+endy/100,xend=-0.52+x/100,yend=-0.5+y/100))+ theme_bw(base_family = font_f) + annotation_raster(pitch, -Inf, Inf, -Inf, Inf) + geom_segment(colour="lightgrey", size=1) + geom_point(pch=21, colour="black", size=1, alpha=0.75, aes(fill=c("homecol"))) + ylim(c(-0.47,0.47)) + xlim(c(-0.47,0.47)) + guides(fill=FALSE)
...
cnt <- qqq %>% count(playername)
cnt <- cnt[order(-cnt$n),]

qqq$playername[qqq$playername != cnt$playername[1]] <- ""

subtitle <- paste("Successful last-third passes: ", nrow(ppp), "-", nrow(qqq), " ", " ", " ", "Most involved: ", cnt$playername[1], "(", cnt$n[1],")")

mm <- ggplot(qqq, aes(x=-0.52+endx/100,y=-0.5+endy/100,xend=-0.52+x/100,yend=-0.5+y/100))+ theme_bw(base_family = font_f) + annotation_raster(pitch, -Inf, Inf, -Inf, Inf) + geom_segment(colour="lightgrey", size=1) + geom_text(size=2, family=font_f, aes(label=playername, x=-0.52+x/100, y=-0.5+y/100)) + geom_point(pch=21, colour="black", size=1, alpha=0.75, aes(fill=c("awaycol"))) + ylim(c(-0.47,0.47)) + xlim(c(-0.47,0.47)) + guides(fill=FALSE)
#mm <- ggplot(qqq, aes(x=-0.52+endx/100,y=-0.5+endy/100,xend=-0.52+x/100,yend=-0.5+y/100))+ theme_bw(base_family = font_f) + annotation_raster(pitch, -Inf, Inf, -Inf, Inf) + geom_segment(colour="lightgrey", size=1) + geom_point(pch=21, colour="black", size=1, alpha=0.75, aes(fill=c("awaycol"))) + ylim(c(-0.47,0.47)) + xlim(c(-0.47,0.47)) + guides(fill=FALSE)

mm <- mm + theme(axis.title.y=element_blank(),axis.text.y = element_blank(),axis.line=element_blank(),axis.text.x=element_blank(),axis.ticks=element_blank(),axis.title.x=element_blank(),legend.position="none",panel.background=element_rect(fill="white"),panel.border=element_blank(),panel.grid.major=element_blank(),panel.grid.minor=element_blank(),plot.background=element_rect(fill="white"))

mm <- mm + scale_fill_manual(values=c(awaycol))
mm <- mm + labs(x=NULL,y=NULL,title=title, subtitle=subtitle)
mm <- mm + theme(plot.title=element_text(hjust=0, face="bold"))


outpath <- paste("output/",title,"home last third.",mid,".jpeg")
ggsave(outpath, plot=gg, width=7, height=4.46)

outpath <- paste("output/",title,"away last third.",mid,".jpeg")
ggsave(outpath, plot=mm, width=7, height=4.46)
