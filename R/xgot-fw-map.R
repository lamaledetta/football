...
print("[info]: Started xg-fw-map R script")
...
qtext <- paste0("
select id, playername, matchid, expandedminute, type, expg, goalmouthy, goalmouthz 
from ", sid,".event 
where
...
#output <- calc_xg(shots)
	...
	situation <- "xgot"
        ...
        data$xgot <- round(predict(model, data, type="response"),3)
        ...
        df$delta <- round(df$goals/df$xgot,2)
...
#title <- pname
subtitle <- paste("On target: ",nrow(data), " ", " ", " ", " ", "Scored: ",sum(data$type), " ", " ", " ", " ", "Expected: ", round(sum(data$xgot),1))
