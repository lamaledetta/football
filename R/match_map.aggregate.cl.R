...
# ARGUMENTS
if (length(args)==0) {
    print(paste0("[ ", info, " ] I need arguments: mid1 mid2 stage homecol awaycol csvpath season"))
    quit("no")
} else {
    mid1    <- args[1]
    mid2    <- args[2]
...
home_team_id <- unique(sha$teamid)[1]
away_team_id <- unique(sha$teamid)[2]
...
outpath <- paste("output/",title,".",mid1,mid2,".jpeg")
