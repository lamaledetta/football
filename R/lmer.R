...
con <- dbConnect(drv,host="localhost",port="5432",dbname="ev")
...
where
    r.year between ", season, " and ", season," ;"))
...
dbWriteTable(con,"_parameter_levels",parameter_levels,row.names=TRUE)
...
dbWriteTable(con,"_basic_factors",as.data.frame(combined),row.names=TRUE)
