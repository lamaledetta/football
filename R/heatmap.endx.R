...
preevents <- round(preevents[,40:41],0)
preplus <- count(round(preevents/10,0)*10,vars=c("endx","endy"))

postevents <- read.csv("/tmp/right.csv")
postmatches <- length(levels(as.factor((postevents$matchid))))
postevents <- round(postevents[,40:41],0)
postplus <- count(round(postevents/10,0)*10,vars=c("endx","endy"))

names(preplus) <- c("x","y","freq")
names(postplus) <- c("x","y","freq")
